...
export function Clientes() {
  ...
  const fetchData = async () => {
    try {
      ...
      toast.error("Error de conexión", { description: "No se pudieron cargar los datos de los clientes." });
      setClients([]);
    } finally {
      setLoading(false);
    }
  ...
  const renderExpirationText = (client) => {
    if (!client.vencimiento_plan) return <span className="text-zinc-600">--</span>;
    ...
    <div className="space-y-6 animate-fade-in-up pb-10">
        ...
        <div>
          <div className="flex items-center gap-4">
            ...
            <div className="bg-gym-orange/10 border border-gym-orange/30 text-gym-orange px-3 py-1.5 rounded-xl flex items-center gap-2 shadow-[0_0_15px_rgba(249,115,22,0.15)] transition-all">
              ...
              <span className="font-black text-xl leading-none">{filteredClients.length}</span>
              <span className="text-[10px] uppercase font-bold tracking-widest opacity-70 mt-0.5">Resultados</span>
            </div>
          </div>
          <p className="text-gym-gray mt-1">Administra perfiles y membresías.</p>
        </div>

        <button onClick={() => { setSelectedClient(null); setIsModalOpen(true); }} className="bg-gym-orange hover:bg-orange-600 text-white px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 shadow-lg transition-all active:scale-95">
      ...
      <div className="bg-gym-card p-4 rounded-xl border border-white/5 flex flex-col md:flex-row items-center gap-4">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          <select className={`border rounded-lg px-3 py-2 text-sm outline-none cursor-pointer w-full md:w-40 transition-colors ${filterStatus !== 'all' ? 'bg-gym-orange/10 border-gym-orange text-gym-orange' : 'bg-black/20 border-white/10 text-white'
            }`} value={filterStatus} onChange={(e) => handleStatusChange(e.target.value)}>
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          <select className="bg-black/20 border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none cursor-pointer w-full md:w-48 transition-colors focus:border-gym-orange" value={sortOrder} onChange={(e) => setSortOrder(e.target.value)}>
        ...
        {(filterStatus !== 'all' || searchTerm !== '' || sortOrder !== 'none') && (
          <button onClick={handleClearFilters} className="p-2 text-red-400 hover:bg-red-500/10 rounded-lg transition-colors" title="Limpiar filtros"><XCircle size={20} /></button>
        )}
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white/5 border-b border-white/5 text-gym-gray text-xs uppercase tracking-wider">
                ...
                <th className="p-4 font-semibold hidden md:table-cell">Entrenador</th>
                <th className="p-4 font-semibold hidden lg:table-cell">Vencimiento</th>
                <th className="p-4 font-semibold text-right">Acciones</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {loading ? (
                <tr><td colSpan="5" className="p-8 text-center text-gym-gray">Cargando datos...</td></tr>
                  ...
                  <td colSpan="5" className="p-12 text-center text-zinc-500">
                    <div className="flex flex-col items-center justify-center gap-2">
                      ...
                      {(filterStatus !== 'all' || searchTerm !== '') && (
                        <button onClick={handleClearFilters} className="text-gym-orange hover:underline text-xs mt-2">Limpiar búsqueda</button>
                      )}
                    </div>
                  </td>
                </tr>
                    ...
                    <tr key={client.id} className="hover:bg-white/5 transition-colors group">
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium text-white group-hover:text-gym-orange transition-colors">{client.nombre}</p>
                            <p className="text-xs text-gym-gray">{client.email}</p>
                            <p className="text-[10px] text-zinc-500 md:hidden mt-0.5">{client.rut}</p>
                          </div>
                        </div>
                      </td>

                      <td className="p-4"><StatusBadge status={realStatus} /></td>

                      <td className="p-4 hidden md:table-cell text-sm text-zinc-300">
                        {client.nombre_entrenador ? <span className="flex items-center gap-1"><Dumbbell size={14} className="text-gym-orange" /> {client.nombre_entrenador}</span> : <span className="opacity-50 italic">Sin asignar</span>}
                      </td>
                      ...
                      <td className="p-4 text-right">
                        <div className="flex justify-end gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                          <button onClick={() => setDetailClient(client)} className="p-2 hover:bg-green-500/10 rounded-lg text-zinc-400 hover:text-green-400 font-bold transition-colors" title="Ver Detalle/Pagos">$</button>
                          <button onClick={() => { setSelectedClient(client); setIsModalOpen(true); }} className="p-2 hover:bg-blue-500/10 rounded-lg text-zinc-400 hover:text-blue-400 transition-colors" title="Editar Perfil"><Edit size={16} /></button>
                          <button onClick={() => handleDelete(client.id)} className="p-2 hover:bg-red-500/10 rounded-lg text-zinc-400 hover:text-red-400 transition-colors" title="Eliminar"><Trash2 size={16} /></button>
                        </div>
                      </td>
                    </tr>
                  )
...
function StatusBadge({ status }) {
  const config = {
    active: { style: 'bg-green-500/10 text-green-400 border-green-500/20', label: 'Vigente', dot: 'bg-green-400' },
    expired: { style: 'bg-red-500/10 text-red-400 border-red-500/20', label: 'Vencido', dot: 'bg-red-500' },
    cancelled: { style: 'bg-red-900/30 text-red-500 border-red-500/50', label: 'Cancelado', dot: 'bg-red-500' },
    none: { style: 'bg-zinc-800/50 text-zinc-400 border-zinc-700', label: 'Sin Plan', dot: 'bg-zinc-500' }
  };
  const { style, label, dot } = config[status] || config.none;

  return (
    <span className={`px-2 py-1 rounded border flex items-center gap-1.5 w-fit text-[10px] font-bold uppercase tracking-wider ${style}`}>
