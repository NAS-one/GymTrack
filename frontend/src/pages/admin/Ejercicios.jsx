...
export function Ejercicios() {
  ...
  const handleSave = async (data) => {
    try {
      if (selectedExercise) await axios.patch(`/ejercicios/${selectedExercise.id}`, data);
      else await axios.post('/ejercicios', data);
      setIsModalOpen(false);
      fetchExercises();
    } catch (e) { alert("Error al guardar: " + (e.response?.data?.error || "Desconocido")); }
  };

  const handleDelete = async (id) => {
    if (!window.confirm("¿Eliminar este ejercicio?")) return;
    try { await axios.delete(`/ejercicios/${id}`); fetchExercises(); } catch (e) { alert("Error al eliminar"); }
  };

  // Filtrado Doble (Grupo + Búsqueda)
  const filtered = exercises.filter(ex => {
    const matchesGroup = filterGroup === 'Todos' || ex.grupo_muscular === filterGroup;
  ...
  return (
    <div className="space-y-6 animate-fade-in-up pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-white">Biblioteca de Ejercicios</h2>
          <p className="text-gym-gray text-sm">Catálogo estandarizado para rutinas de entrenamiento.</p>
        </div>
        <button onClick={() => { setSelectedExercise(null); setIsModalOpen(true); }} className="bg-gym-orange hover:bg-orange-600 text-white px-4 py-2.5 rounded-xl font-medium flex items-center gap-2 shadow-lg transition-all">
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="Buscar ejercicio..." className="w-full bg-gym-card border border-white/10 rounded-xl pl-10 pr-4 py-3 text-white focus:border-gym-orange outline-none transition-colors" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
      </div>

      {/* GRID DE EJERCICIOS */}
      {loading ? (
        <div className="text-center py-12 text-gym-gray">Cargando biblioteca...</div>
      ) : (
      ...
      <ExerciseModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} exercise={selectedExercise} onSave={handleSave} />
    </div>
  );
}