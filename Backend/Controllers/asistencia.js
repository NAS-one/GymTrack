...
export class AsistenciaController {
  ...
  registrarAcceso = async (req, res) => {
    ...
    try {
      const resultado =
        await this.AsistenciaModel.registrarNuevoAcceso(identificador);

      if (resultado.estado_acceso === "aprobado") {
        //Usamos "ingreso_exitoso" para que el toast sea Verde
        dispararAlertaStaff(
          "Acceso Permitido",
          `${resultado.nombre} ha ingresado correctamente.`,
          "ingreso_exitoso",
        );
        return success(req, res, resultado, 200);
      }

      //Usamos "ingreso_denegado" para que el toast sea Rojo
      dispararAlertaStaff(
        "ACCESO DENEGADO",
        `${resultado.nombre}: ${resultado.mensaje || "Consulte en recepción"}`,
        "ingreso_denegado",
      );
      ...
      if (e.message === "USUARIO_NO_ENCONTRADO") {
        //Usamos "alerta_sistema" para Naranja/Advertencia
        dispararAlertaStaff(
          "USUARIO DESCONOCIDO",
          `Intento de acceso con identificador no registrado: ${identificador}`,
          "alerta_sistema",
        );
  ...
  scanCheckIn = async (req, res) => {
    ...
    if (lastScanTime && now - lastScanTime < FIVE_MINUTES) {
      return success(
        ...
        res,
        {
          estado_acceso: "denegado",
          mensaje:
            "Ya has registrado tu acceso recientemente. Espera 5 minutos.",
        },
        200,
      );
    }
    ...
    try {
      ...
      const resultado = await this.AsistenciaModel.registrarNuevoAcceso(
        id_usuario.toString(),
      );
      ...
      if (resultado.estado_acceso === "aprobado") {
        ...
        dispararAlertaStaff(
          "Nuevo Ingreso",
          `${resultado.nombre} acaba de hacer check-in. ¡Pase libre!`,
          "ingreso_exitoso",
        );
      } else {
        // 🌟 CORREGIDO
        dispararAlertaStaff(
          "Acceso Denegado",
          `${resultado.nombre} intentó ingresar. ${resultado.mensaje}`,
          "ingreso_denegado",
        );
      }
  ...
  getAll = async (req, res) => {
    try {
      ...
      const logs = await this.AsistenciaModel.getAll({
        year,
        month,
        day,
        type,
