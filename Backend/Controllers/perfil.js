...
export class PerfilController {
  ...
  getMyProfile = async (req, res) => {
    try {
      ...
      // IMPORTANTE: Nunca devolver el hash de la contraseña al frontend
...
// 2. Actualizar Mis Datos Personales
  updateMyProfile = async (req, res) => {
    ...
    if (!result.success) return error(req, res, JSON.parse(result.error.message), 400);

    try {
      const id_usuario = req.user.id;
      
    
      // A veces Zod elimina campos si no están bien definidos en el schema, así lo aseguramos:
  ...
  updateMyPassword = async (req, res) => {
    ...
    try {
      ...
      success(req, res, { message: "Contraseña actualizada exitosamente" }, 200);
    } catch (e) {
      console.error(e);
  ...
  //  5. Actualizar Preferencias (JSONB)
  updatePreferencias = async (req, res) => {
    try {
      ...
      const updatedAdmin = await this.PerfilModel.updatePreferencias(id_usuario, preferencias);
