...
export class ColaboradorController {
  ...
  getStats = async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Error al calcular estadísticas: " + e.message });
    }
  };
  ...
  handleDbError(e, res, action) {
    ...
    if (e.code === "23505") {
      ...
      if (info.includes("id_usuario"))
        return res
          .status(409)
          .json({ error: "Este usuario ya tiene un perfil asociado." });

      return res
        .status(409)
        .json({ error: "Error de duplicidad (dato ya existe)." });
    }
