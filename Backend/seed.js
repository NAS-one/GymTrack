...
async function seed() {
  console.log("🌱 Iniciando Siembra ENTERPRISE 4.0 (con módulo de entrenador completo)...");
  ...
  try {
    ...
    console.log(`   👤 admin          | email: ${DEV_EMAIL_ADMIN}    | pass: 123456`);
    console.log(`   👤 entrenador_dev | email: ${DEV_EMAIL_TRAINER} | pass: 123456`);
    console.log(`   👤 cliente_dev    | email: ${DEV_EMAIL_CLIENT}  | pass: 123456`);
    ...
    for (const c of coachesData) {
      ...
      trainerUserIds.push({ id_entrenador: t.id, id_usuario: u.id, turno: c.turno });
    ...
    for (let i = 1; i <= 150; i++) {
      ...
      const assignedTrainer = Math.random() > 0.4 ? getRandomItem(allTrainerIds) : null;
      ...
      clientPool.push({
        id_cliente: c.id, id_usuario: u.id, nombre,
        activeUntil: new Date(2024, 11, 31), assignedTrainer,
        habitShift, attendanceProb,
        currentWeight: baseWeight, currentFat: baseFat,
        targetWeight: baseWeight * (isMale ? 1.05 : 0.9), height: baseHeight
      });
    }
    ...
    // =========================================================
    console.log("📅 Generando sesiones de entrenadores (6 meses histórico + próximas)...");
    ...
    for (const trainerId of allTrainerIds) {
      const misClientes = clientesConEntrenador.filter(c => c.id_entrenador === trainerId);
      if (misClientes.length === 0) continue;

      // Porcentaje del entrenador (usamos 0.35 como default para los de sueldo fijo también)
      const [trainerInfo] = await sql`SELECT porcentaje_retencion FROM entrenadores WHERE id = ${trainerId}`;
      const porcentajeEntrenador = trainerInfo?.porcentaje_retencion > 0 ? (1 - Number(trainerInfo.porcentaje_retencion)) : 0.6;

      for (let mesesAtras = 5; mesesAtras >= 0; mesesAtras--) {
        const cantidadSesiones = getRandomInt(6, Math.min(18, misClientes.length * 3));
        for (let s = 0; s < cantidadSesiones; s++) {
          ...
          const estado = mesesAtras > 0 ? (Math.random() < 0.9 ? 'realizada' : 'cancelada') : 
                         (fecha < new Date() ? 'realizada' : 'agendada');
          sesionesBuffer.push({
    ...
    const ejerciciosDb = [
        ...
        { nombre: "Dominadas Supinas", grupo_muscular: "Espalda", descripcion: "Ejercicio de tracción vertical para dorsales y bíceps." },
    ...
    console.log(`   entrenador_dev → entrenador_dev / 123456   (${DEV_EMAIL_TRAINER})`);
    console.log(`   cliente_dev    → cliente_dev / 123456   (${DEV_EMAIL_CLIENT})`);
    console.log("==========================================");
